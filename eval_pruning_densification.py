...
CONFIG = dict(
    ...
    # --- the 3 relevant parameters for densification and pruning ---
    densify_until_iter=0,
    densify_grad_threshold=0.0002,
    opacity_cull=0.005,
)

# --- DEFINE THE ABLATIONS ---
ABLATION_MODES = {
    "no_densify_no_prune": {
        "densify_until_iter": 0,       # Turn off the function entirely
        "densify_grad_threshold": 1.0, # Irrelevant
        "opacity_cull": 0.0            # Irrelevant
    },
    "prune_only": {
        "densify_until_iter": 7000,    # Function must run
        "densify_grad_threshold": 1000.0, # IMPOSSIBLE threshold -> No Densification
        "opacity_cull": 0.005          # Standard Pruning ON
    },
    "densify_only": {
        "densify_until_iter": 7000,    # Function must run
        "densify_grad_threshold": 0.0002, # Standard Densification ON
        "opacity_cull": 0.0            # 0.0 opacity -> No Pruning
    },
    "densify_and_prune": {                 # Control group (Both ON)
